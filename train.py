...
def _scalar_to_device(t, ref, dtype=None):
    if dtype is None:
        dtype = ref.dtype
...
while True:
    ...
    mfu = 0.0 if not math.isfinite(peak_flops) else 100 * num_flops_per_token * TOTAL_BATCH_SIZE / dt / peak_flops
...
steady_state_mfu = 0.0 if (total_training_time <= 0 or not math.isfinite(peak_flops)) else 100 * num_flops_per_token * TOTAL_BATCH_SIZE * (step - 10) / total_training_time / peak_flops
