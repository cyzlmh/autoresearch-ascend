...
You can override the cache root with AUTORESEARCH_CACHE_DIR.
"""
...
_CACHE_DIR_ENV = os.environ.get("AUTORESEARCH_CACHE_DIR", "").strip()
if _CACHE_DIR_ENV:
    CACHE_DIR = os.path.abspath(os.path.expanduser(_CACHE_DIR_ENV))
else:
    CACHE_DIR = os.path.join(os.path.expanduser("~"), ".cache", "autoresearch")
DATA_DIR = os.path.join(CACHE_DIR, "data")
TOKENIZER_DIR = os.path.join(CACHE_DIR, "tokenizer")
DATASET_PATH = "/datasets/karpathy/climbmix-400b-shuffle/resolve/main"
DEFAULT_BASE_URL = f"https://huggingface.co{DATASET_PATH}"
...
def _build_data_base_urls():
    """
    Build candidate base URLs for shard downloads.

    Priority:
    1) AUTORESEARCH_DATA_BASE_URL (full dataset base URL)
    2) HF_ENDPOINT + fixed dataset path
    3) Default huggingface.co dataset URL
    """
    explicit_base = _normalize_base_url(os.environ.get("AUTORESEARCH_DATA_BASE_URL", ""))
    if explicit_base:
        return [explicit_base]

    candidates = []
    hf_endpoint = _normalize_base_url(os.environ.get("HF_ENDPOINT", ""))
    if hf_endpoint:
        if "/datasets/" in hf_endpoint:
            candidates.append(hf_endpoint)
        else:
            candidates.append(f"{hf_endpoint}{DATASET_PATH}")
...
BASE_URLS = _build_data_base_urls()
